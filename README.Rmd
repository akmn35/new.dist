...
```demd``` Density function for Maxwell distribution with scale (theta) paramete.
```{r}
library(new.dist)
demd(1,theta=2)
```

```pemd``` Distribution function for a Maxwell Distribution with scale (theta) paramete.
```{r}
library(new.dist)
pemd(1,theta=2)
```
...
```dndd``` Density function for a New Discrete Distribution with theta parameter.
```{r}
library(new.dist)
dndd(2,theta=2)
```

```pndd``` Distribution function for a New Discrete Distribution with theta parameter.
```{r}
library(new.dist)
pndd(1,theta=2)
```

```qndd``` Quantile function for a New Discrete Distribution with theta parameter.
```{r}
library(new.dist)
qndd(.5,theta=2)
```

```rndd``` Random generation for a New Discrete Distribution with theta parameter.
```{r}
library(new.dist)
  rndd(5,theta=1)
```
