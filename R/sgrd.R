...
#' @description
#' Density, distribution function, quantile function and random generation for the Slashed generalized Rayleigh distribution with parameters \code{shape}, \code{scale}, \code{kurtosis}.
...
#' @details
#' The Slashed generalized Rayleigh distribution with \code{shape} parameter \eqn{\alpha}, \code{scale} parameter \eqn{\theta} and \code{kurtosis} parameter \eqn{q}, has density given by
#' \deqn{f\left( x\right) =\frac{qx^{-\left( q+1\right) }}{\Gamma \left( \alpha+1\right) \theta ^{q/2}}\Gamma \left( \frac{2\alpha +q+2}{2}\right) F\left(\theta x^{2};\frac{2\alpha +q+2}{2},1\right), }
...
dsgrd <- function(x,theta,alpha,q,log=FALSE)
  {
    ...
    if(any(q<=2)) {(stop("the function may not work when q<2 since the first and second moment does not exist!"))}
    ...
    for (i in 1:enuzun)suppressWarnings(
    {
      ...
      {pdf[i]<-q[i]*x[i]^(-(q[i]+1))/(gamma(alpha[i]+1)*theta[i]^(q[i]/2))*gamma((2*alpha[i]+q[i]+2)/2)*stats::pgamma(theta[i]*x[i]^2,(2*alpha[i]+q[i]+2)/2,1)}
    })
...
psgrd<-function(x,theta,alpha,q,lower.tail=TRUE,log.p=FALSE)
  ...
  if(any(theta<=0)) {stop("theta must be > 0")}
    ...
    for (i in 1:enuzun)suppressWarnings(
    {
      if(x[i]>0){integral[i] <- stats::integrate(function(t) {q[i]*t^(-(q[i]+1))/(gamma(alpha[i]+1)*theta[i]^(q[i]/2))*gamma((2*alpha[i]+q[i]+2)/2)*stats::pgamma(theta[i]*t^2,(2*alpha[i]+q[i]+2)/2,1)
...
qsgrd<-function(p,theta,alpha,q,lower.tail=TRUE)
{
  ...
  for (i in 1:enuzun)suppressWarnings(
  {
    ...
    kok[i]<-(stats::optim(Ex,F,method="Brent",lower=0,upper=20))$par
  })
  return(kok)
}
#' Slashed generalized Rayleigh distribution
