...
#' @param sigma the parameter that controls the uni- or bimodality of the distribution.
...
#' @description
#' Density, distribution function, quantile function and random generation for a Bimodal Weibull Distribution with parameters \code{shape} and \code{scale}.
...
#' @details
#' A Bimodal Weibull distribution with \code{shape} parameter \eqn{\alpha}, \code{scale} parameter \eqn{\beta} and the parameter that \code{controls} the uni- or bimodality of the distribution \eqn{\sigma}, has density given by
#' \deqn{f\left( x\right) =\frac{\alpha }{\beta Z_{\theta }}\left[ 1+\left( 1-\sigma~x\right) ^{2}\right] \left( \frac{x}{\beta }\right) ^{\alpha -1}\exp \left( -\left( \frac{x}{\beta }\right) ^{\alpha }\right), }
#' where
#' \deqn{Z_{\theta }=2+\sigma ^{2}\beta ^{2}\Gamma \left( 1+\left( 2/\alpha \right)\right) -2\sigma \beta \Gamma \left( 1+\left( 1/\alpha \right) \right) }
#' and
...
qbwd<-function(p,alpha,beta=1,sigma,lower.tail=TRUE)
  {
    ...
    for (i in 1:enuzun)suppressWarnings(
    {
      Ex<-beta[i]*(2*gamma(1+(1/alpha[i]))+sigma[i]^2*beta[i]^2*gamma(1+(3/alpha[i]))-2*sigma[i]*beta[i]*gamma(1+(2/alpha[i])))/(2+sigma[i]^2*beta[i]^2*gamma(1+(2/alpha[i]))-2*sigma[i]*beta[i]*gamma(1+(1/alpha[i])))
      F<-function(x)
      {
        abs(((2-(1+(1-sigma[i]*x)^2)*exp(-(x/beta[i])^alpha[i])-((2*sigma[i]*beta[i])/alpha[i])*(pracma::gammainc(x^alpha[i]/beta[i]^alpha[i],1/alpha[i])[1]-sigma[i]*beta[i]*                                                                                             pracma::gammainc(x^alpha[i]/beta[i]^alpha[i],2/alpha[i])[1]))/(2+sigma[i]^2*beta[i]^2*gamma(1+(2/alpha[i]))-2*sigma[i]*beta[i]*gamma(1+(1/alpha[i]))))-p[i])
      }
      if(lower.tail==FALSE)
      {
        F<-function(x)
        {
          abs(((2-(1+(1-sigma[i]*x)^2)*exp(-(x/beta[i])^alpha[i])-((2*sigma[i]*beta[i])/alpha[i])*(pracma::gammainc(x^alpha[i]/beta[i]^alpha[i],1/alpha[i])[1]-sigma[i]*beta[i]*                                                                                                     pracma::gammainc(x^alpha[i]/beta[i]^alpha[i],2/alpha[i])[1]))/(2+sigma[i]^2*beta[i]^2*gamma(1+(2/alpha[i]))-2*sigma[i]*beta[i]*gamma(1+(1/alpha[i]))))-(1-p[i]))
        }
      }
