...
test_that("functions returns a list", {
  ...
  expect_type(dwbgd(1,alpha=.1,lambda=1), "double")
  expect_type(pwbgd(1,alpha=.1,lambda=1), "double")
  expect_type(qwbgd(.1,alpha=.1,lambda=1), "double")
  expect_type(rwbgd(1,alpha=.1,lambda=1), "double")
  ...
  expect_type(qgld(.1, a=1, alpha=1, beta=1), "double")
...
test_that("functions returns errors", {
  ...
  expect_error(dsgrd(2,3,1,-4), "the function may not work when q<2 since the first and second moment does not exist!")
  expect_error(psgrd(2,3,-2,4), "alpha must be > -1")
  expect_error(psgrd(2,-3,1,4), "theta must be > 0")
  expect_error(psgrd(2,3,1,-4), "the function may not work when q<2 since the first and second moment does not exist!")
  ...
  expect_error(qsgrd(0.5,3,1,-4), "the function may not work when q<2 since the first and second moment does not exist!")
  ...
  expect_error(rsgrd(2,3,1,-4), "the function may not work when q<2 since the first and second moment does not exist!")
  ...
  expect_error(dwbgd(1, alpha=2, lambda=3), "alpha must be between >= 0 and <= 1")
  expect_error(dwbgd(1, alpha=.2, lambda=-3), "lambda must be > 0")
  expect_error(pwbgd(1, alpha=2, lambda=3), "alpha must be between >= 0 and <= 1")
  expect_error(pwbgd(1, alpha=.2, lambda=-3), "lambda must be > 0")
  expect_error(qwbgd(-1, alpha=2, lambda=3), "p must be between >= 0 and <= 1")
  expect_error(qwbgd(.1, alpha=2, lambda=3), "alpha must be between >= 0 and <= 1")
  expect_error(qwbgd(.1, alpha=.2, lambda=-3), "lambda must be > 0")
  expect_error(rwbgd(-1, alpha=2, lambda=3), "n must be >= 1")
  expect_error(rwbgd(1, alpha=2, lambda=3), "alpha must be between >= 0 and <= 1")
  expect_error(rwbgd(1, alpha=.2, lambda=-3), "lambda must be > 0")
  ...
  expect_error(qpldd(2, alpha=-2, beta=3, theta=4), "p must be between >= 0 and <= 1")
...
test_that("functions returns a  vector with the expected size", {
  ...
  expect_vector(dbwd(-10, alpha=2, sigma=2), ptype = double(), size = 1)
  expect_vector(dbwd(10, alpha=2, sigma=2, log = T), ptype = double(), size = 1)
  expect_vector(pbwd(10, alpha=2, sigma=2), ptype = double(), size = 1)
  expect_vector(pbwd(10, alpha=2, sigma=2, log.p = T), ptype = double(), size = 1)
  expect_vector(pbwd(-10, alpha=2, sigma=2), ptype = double(), size = 1)
  expect_vector(pbwd(10, alpha=2, sigma=2, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qbwd(.10, alpha=2, sigma=2, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(dsgrd(-2, theta=3, alpha=1, q=4), ptype = double(), size = 1)
  expect_vector(dsgrd(2, theta=3, alpha=1, q=4, log = T), ptype = double(), size = 1)
  expect_vector(psgrd(2, theta=3, alpha=1,q =4), ptype = double(), size = 1)
  expect_vector(psgrd(-2, theta=3, alpha=1,q =4), ptype = double(), size = 1)
  expect_vector(psgrd(2, theta=3, alpha=1, q =4, log.p = T), ptype = double(), size = 1)
  expect_vector(psgrd(2, theta=3, alpha=1, q=4, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qsgrd(.2,theta=3,alpha=1,q=4), ptype = double(), size = 1)
  expect_vector(qsgrd(.2, theta=3, alpha=1, q=4, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(psod(0.4, alpha=1, beta=2, log.p = T), ptype = double(), size = 1)
  expect_vector(psod(0.4, alpha=1, beta=2, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qsod(0.4, alpha=1, beta=2), ptype = double(), size = 1)
  expect_vector(qsod(0.4, alpha=1, beta=2, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(ptpmd(-1, beta=2, alpha=3), ptype = double(), size = 1)
  expect_vector(ptpmd(1, beta=2, alpha=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(ptpmd(1, beta=2, alpha=3, log.p = T), ptype = double(), size = 1)
  expect_vector(qtpmd(.1, beta=2, alpha=3), ptype = double(), size = 1)
  expect_vector(qtpmd(.1, beta=2, alpha=3,lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(ptprd(5, lambda=4, mu=4, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qtprd(.5, lambda=4, mu=4), ptype = double(), size = 1)
  expect_vector(qtprd(.5, lambda=4, mu=4, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(puigd(-1, mu=2, lambda=3), ptype = double(), size = 1)
  expect_vector(puigd(1, mu=2, lambda=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(puigd(1, mu=2, lambda=3, log.p = T), ptype = double(), size = 1)
  expect_vector(quigd(.1, mu=2, lambda=3), ptype = double(), size = 1)
  expect_vector(quigd(.1, mu=2, lambda=3,lower.tail = F), ptype = double(), size = 1)
  expect_vector(ruigd(20, mu=2, lambda=3), ptype = double(), size = 20)
  expect_vector(dwbgd(1, alpha=.2, lambda=3), ptype = double(), size = 1)
  expect_vector(dwbgd(-1, alpha=.2, lambda=3), ptype = double(), size = 1)
  expect_vector(dwbgd(1, alpha=.2, lambda=3, log=T), ptype = double(), size = 1)
  expect_vector(pwbgd(1, alpha=.2, lambda=3), ptype = double(), size = 1)
  expect_vector(pwbgd(1, alpha=.2, lambda=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(pwbgd(1, alpha=.2, lambda=3, log.p = T), ptype = double(), size = 1)
  expect_vector(qwbgd(.1, alpha=.2, lambda=3), ptype = double(), size = 1)
  expect_vector(qwbgd(.1, alpha=.2, lambda=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(rwbgd(20, alpha=.2, lambda=3), ptype = double(), size = 20)
  ...
  expect_vector(qdld(0.1, theta=0.5, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(depkd(0.1, lambda=2, alpha=3, log = T), ptype = double(), size = 1)
  expect_vector(pepkd(0.1, lambda=2, alpha=3), ptype = double(), size = 1)
  expect_vector(pepkd(0.1, lambda=2, alpha=3, log.p = T), ptype = double(), size = 1)
  expect_vector(pepkd(0.1, lambda=2, alpha=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qepkd(0.1, lambda=2, alpha=3), ptype = double(), size = 1)
  expect_vector(qepkd(0.1, lambda=2, alpha=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(repkd(20, lambda=2,alpha=3), ptype = double(), size = 20)
  expect_vector(dgld(1, a=2, alpha=3, beta=4), ptype = double(), size = 1)
  expect_vector(dgld(-1, a=2, alpha=3, beta=4), ptype = double(), size = 1)
  expect_vector(dgld(1, a=2, alpha=3, beta=4, log = T), ptype = double(), size = 1)
  expect_vector(pgld(1, a=2, alpha=3, beta=4), ptype = double(), size = 1)
  expect_vector(pgld(-1, a=2, alpha=3, beta=4), ptype = double(), size = 1)
  expect_vector(pgld(1, a=2, alpha=3, beta=4, log.p = T), ptype = double(), size = 1)
  expect_vector(pgld(1, a=2, alpha=3, beta=4, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qgld(.1, a=2, alpha=3, beta=4), ptype = double(), size = 1)
  expect_vector(qgld(.1, a=2, alpha=3, beta=4, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(qndd(c(.5,.2),theta=c(2,3),lower.tail=F), ptype = double(), size = 2)
  ...
  expect_vector(pnld(-1, lambda=2, beta=3), ptype = double(), size = 1)
  expect_vector(pnld(1, lambda=2, beta=3, log.p = T), ptype = double(), size = 1)
  expect_vector(pnld(1, lambda=2, beta=3, lower.tail = F), ptype = double(), size = 1)
  expect_vector(qnld(.1, lambda=2, beta=3), ptype = double(), size = 1)
  expect_vector(qnld(.1, lambda=2, beta=3, lower.tail = F), ptype = double(), size = 1)
  ...
  expect_vector(dpldd(1, alpha=2, beta=3, theta=4, log = T), ptype = double(), size = 1)
  expect_vector(ppldd(1, alpha=2, beta=3, theta=4), ptype = double(), size = 1)
  expect_vector(ppldd(1, alpha=2, beta=3, theta=4, lower.tail = F), ptype = double(), size = 1)
  expect_vector(ppldd(1, alpha=2, beta=3, theta=4, log.p = T), ptype = double(), size = 1)
  expect_vector(qpldd(.1, alpha=2, beta=3, theta=4), ptype = double(), size = 1)
  expect_vector(qpldd(.1, alpha=2, beta=3, theta=4, lower.tail = F), ptype = double(), size = 1)
  expect_vector(rpldd(20, alpha=2, beta=3, theta=4), ptype = double(), size = 20)
